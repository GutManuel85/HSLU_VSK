...
public final class StandaloneGameOfLife extends GameOfLife {
    ...
    public static LoggerSetup loggerSetup;
    public static Logger LOGGER;
    private Frame appletFrame;
    private String[] args;
    private GameOfLifeGridIO gridIO;

    private static List<Object> loggerSetupConfiguration(String configFileName){
        ...
        String loggerName = "GoL-" + Thread.currentThread().getName();
        String serverIP = "localhost";
        int serverPort = 1142;
        LogLevel minimumLogLevel = LogLevel.TRACE;
        String loggerSetupImplementation = "ch.hslu.vsk.logger.component.LoggerSetup";
        ArrayList<Object> args = new ArrayList<>();
        if (prop != null){
            if (prop.getProperty("LOGGERNAME") != null){
            ...
            if (prop.getProperty("MINIMUMLOGLEVEL") != null){
                String level = prop.getProperty("MINIMUMLOGLEVEL");
            ...
            if (prop.getProperty("LOGGERSETUP_CLASS") != null){
                loggerSetupImplementation = prop.getProperty("LOGGERSETUP_CLASS");
            }
        }
        args.add(loggerName);
        args.add(serverIP);
        args.add(serverPort);
        args.add(minimumLogLevel);
        args.add(loggerSetupImplementation);
        return args;
    ...
    public static void main(String args[]) throws ClassNotFoundException {
        ...
        String configFileName = "logger.config";
        String workingDirectory = System.getProperty("user.dir");

        if (args.length == 0){
        }
        else if (args.length == 1){
            configFileName = ((String) args[0]);
        }

        List<Object> argsStandard = loggerSetupConfiguration(configFileName);

        String loggerName = (String) argsStandard.get(0);
        String serverIP = (String) argsStandard.get(1);
        int serverPort = (Integer) argsStandard.get(2);
        LogLevel minimumLogLevel = (LogLevel) argsStandard.get(3);
        String loggerSetupImplementation = (String) argsStandard.get(4);


        loggerSetup = LoggerSetupFactory.createLoggerSetup(loggerSetupImplementation);
        loggerSetup.setLoggerName(loggerName);
        loggerSetup.setMinimumLevel(minimumLogLevel);
